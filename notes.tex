...
Множество, из элементов которого составляют конкретное множество, называют \textbf{универсальным} и обозначается символом \(U\).
...
Под определению пустое множество является подмножеств всех множеств:
\[
...
\[
    ...
    \iff
    \begin{dcases}
        \forall a \in A \rightarrowtail !b \in B \\
        \forall b \in B \rightarrowtail !a \in A
    \end{dcases}
\]
...
\[
    \exists B: B \subseteq A \land |B| = |A| \land B = A.
\]
...
Теорема Кантора о несчетности:

Отрезок \([0, 1]\) несчетен, т. е. \(|[0, 1]| > |\mathbb{N}|\).
...
\textbf{Пересечение} двух множеств называется множество, состоящее из элементов входящих в каждое из множеств \(A\) и \(B\):
\[
